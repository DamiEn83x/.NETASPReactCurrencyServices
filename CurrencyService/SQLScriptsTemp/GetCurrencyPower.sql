...
declare @Datefrom dateTime = '2022-01-01',
		...
		@RefCurrencyCodes nvarchar(max) ='THB,ZMW',
		...
		select cr.DateOfRate,sum(r.bucketamount)/sum(1/cr.RateToBaseCurrency) as valueindex from #ReferenceList  r join  Currencies c on r.Code=c.code join  CurrencyRates cr on  cr.CurrencyId=c.CurrencyId  where cr.DateOfRate>=@Datefrom and cr.DateOfRate<=@DateTo group by cr.DateOfRate order by DateOfRate
